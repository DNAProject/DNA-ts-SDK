 ...
 * @param issuer Issuer's DNA ID
 * @param subject Subject's DNA ID
 ...
 *
 * @param claimId Unique id of the claim
 * @param revokerDNAid Revoker's DNA ID
 * @param gasPrice Gas price
 * @param gasLimit Gas limit
 * @param payer Payer's address
 */
export function buildRevokeRecordTx(claimId: string, revokerDNAid: string,
                                    gasPrice: string, gasLimit: string, payer: Address) {
    ...
    if (revokerDNAid.substr(0, 3) === 'did') {
        revokerDNAid = str2hexstr(revokerDNAid);
    }

    const p1 = new Parameter(name1, type1, str2hexstr(claimId));
    const p2 = new Parameter(f.parameters[1].getName(), ParameterType.ByteArray, revokerDNAid);
