 ...
 * @param dnaid user's DNA ID, must be assigned with the role.
 * @param peerPubKey public key of user's peer
 * @param userAddr user's address to pledge DNA&ONG. This address must have enough DNA & ONG.
...
export function makeRegisterCandidateTx(
    ...
    struct.add(str2hexstr(peerPubKey), userAddr.serialize(), initPos, dnaid, keyNo);
...
/**
 *
 * @param userAddr User's address to pledge DNA&ONG.
 * @param peerPubKey Public key of user's peer
 * @param payer Address to pay for the gas.
 * @param gasPrice Gas price
 * @param gasLimit Gas limit
 ...
 * @param posList amount of DNA to unvote
 * @param payer Address to pay for the gas.
 * @param gasPrice Gas price
 * @param gasLimit Gas limit
 */
