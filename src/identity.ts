...
export class Identity {
    ...
    static importIdentity(
        ...
        identity.dnaid = Address.generateDNAid(publicKey);
        identity.label = label;
        identity.lock = false;
        identity.isDefault = false;

        // control
        const control = new ControlData('1', encryptedPrivateKey, Address.fromDNAid(identity.dnaid), saltBase64);
    ...
    static create(privateKey: PrivateKey, keyphrase: string, label: string, params?: ScryptParams) {
        ...
        const publicKey = privateKey.getPublicKey();
        identity.dnaid = Address.generateDNAid(publicKey);
        const address = Address.fromDNAid(identity.dnaid);
