...
import { buildGetDDOTx, buildGetPublicKeyStateTx } from '../smartcontract/nativevm/idContractTxBuilder';
...
export abstract class Message {
     ...
     *
     * @param url Restful endpoint of DNA node
    ...
    /**
     * Verifies the signature and check ownership of specified DNA ID through smart contract call.
     *
     * @param url Restful endpoint of DNA node
    ...
    private verifyKeyOwnership(): boolean {
        ...
        if (signature !== undefined && signature.publicKeyId !== undefined) {
            const dnaid = extractDNAId(signature.publicKeyId);

            return dnaid === this.metadata.issuer;
        } else {
            return false;
        }
    }
...
/**
 * Gets the public key associated with DNA ID from blockchain.
 *
 * @param publicKeyId The ID of a signature public key
 * @param url Restful endpoint of DNA node
 */
export async function retrievePublicKey(publicKeyId: string, url: string): Promise<PublicKey> {
    const dnaid = extractDNAId(publicKeyId);
...
/**
 * Gets the state of public key associated with DNA ID from blockchain.
 *
 * @param publicKeyId The ID of a signature public key
 * @param url Restful endpoint of DNA node
 */
export async function retrievePublicKeyState(publicKeyId: string, url: string): Promise<PublicKeyStatus> {
    ...
    const client = new RestClient(url);
    const tx = buildGetPublicKeyStateTx(dnaid, keyId);
    const response = await client.sendRawTransaction(tx.serialize(), true);
...
/**
 * Extracts DNA ID from public key Id.
 *
 * @param publicKeyId The ID of a signature public key
 */
export function extractDNAId(publicKeyId: string): string {
