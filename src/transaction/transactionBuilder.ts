...
import { makeTransferTx } from '../smartcontract/nativevm/assetTxBuilder';
import { buildGetDDOTx, buildRegisterDNAidTx } from '../smartcontract/nativevm/idContractTxBuilder';
...
export function buildNativeTxFromJson(json: any) {
    ...
    if (json.contractHash.indexOf('02') > -1 || json.contractHash.indexOf('01') > -1) { // DNA ONG contract
        const tokenType = json.contractHash.indexOf('02') > -1 ? 'ONG' : 'DNA';
    ...
    } else if (json.contractHash.indexOf('03') > -1) { // DNA ID contract
        if (funcArgs.operation === 'regIDWithPublicKey') {
            const dnaid = args[0].value.substr(args[0].value.indexOf(':') + 1);
            const pk = new PublicKey(args[1].value.split(':')[1]);
            const payer = new Address(json.payer);
            const tx = buildRegisterDNAidTx(dnaid, pk, json.gasPrice, json.gasLimit, payer);
            return tx;
