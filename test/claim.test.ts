...
import { buildRegisterDNAidTx } from '../src/smartcontract/nativevm/idContractTxBuilder';
...
describe('test claim', () => {
    ...
    const dnaid =  identity.dnaid;
    const address = account.address;
    const publicKeyId = dnaid + '#keys-1';
    ...
    /**
     * Registers new DNA ID to create transaction with Events and new block
     */
    beforeAll(async () => {
        const tx = buildRegisterDNAidTx(dnaid, publicKey, '500', '30000');
    ...
    test('test deserialization', async () => {
        ...
        expect(msg.metadata.issuer).toEqual(dnaid);
        expect(msg.metadata.subject).toEqual(dnaid);
    ...
    test('claim', async () => {
        const restUrl = 'http://polaris1.ont.io:20334';
        const dnaid = 'did:dna:AN88DMMBZr5X9ChpMHX3LqRvQHqGxk2c3r';
        const publicKeyId = dnaid + '#keys-1';
